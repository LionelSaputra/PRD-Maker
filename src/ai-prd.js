...
async function callRouter(routerCfg, chosenModel, systemPrompt, userPrompt, attempt = 1, busyIdx = 0, omitJsonMode = false) {
  ...
  if (!res.ok) {
    ...
    // Kegagalan sementara (5xx / "reset after Ns" / connect timeout) = router
    // melepas koneksi ke provider, BUKAN bukti request kita terlalu lambat:
    // terukur live, panggilan KECIL seperti tahap fitur pun kena "reset after
    // 30s", lalu percobaan berikutnya di menit lain sukses. Karena model
    // pilihan pengguna di-pin (tanpa fallback antar model), satu-satunya jalur
    // selamat adalah menunggu lebih sabar: 5 percobaan, ladder 5-60 dtk.
    // Skeleton 5 tahap (~90-300 dtk kerja) tidak boleh dibuang karena blip.
    // Skala jeda bisa dikecilkan untuk test (PRDMAKER_BACKOFF_SCALE).
    const transient = [500, 502, 503, 504].includes(res.status) || /connect timeout|reset after|Timeout|ETIMEDOUT/i.test(rawText);
    if (transient && attempt <= 5) {
      const scale = Number(process.env.PRDMAKER_BACKOFF_SCALE) || 1;
      const backoffMs = Math.round([5000, 15000, 30000, 45000, 60000][attempt - 1] * scale);
      console.warn(`[AI] Router HTTP ${res.status} (sementara), mengulang ${attempt}/5 setelah jeda ${Math.round(backoffMs / 1000)} dtk...`);
